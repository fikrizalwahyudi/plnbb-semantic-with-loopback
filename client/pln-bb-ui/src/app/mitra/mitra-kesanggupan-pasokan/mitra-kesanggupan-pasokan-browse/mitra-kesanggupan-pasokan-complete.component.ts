...
@Component({
  selector: 'app-mitra-kesanggupan-pasokan-complete',
  template: `
...
export class MitraKesanggupanPasokanCompleteComponent implements OnInit {
  ...
  constructor(
    ...
    this.mitraApi.findOne({where: {userId: this.userApi.getCurrentId()}}).subscribe((mitra:Mitra) => {
      this.shippingApi.find({where: {mitraId: mitra.id, status: 3}, include: ['jetty', 'mitra', 'referensiKontrak', 'si', 'transport', 'tujuanPltu', 'loading']}).subscribe(data => {
