...
@Component({
  selector: 'app-mitra-kesanggupan-pasokan-delayed',
  template: `
...
export class MitraKesanggupanPasokanDelayedComponent implements OnInit {
  ...
  constructor(
    ...
    this.mitraApi.findOne({where: {userId: this.userApi.getCurrentId()}}).subscribe((mitra:Mitra) => {
      this.shippingApi.find({where: {mitraId: mitra.id, laycanEndDate: {lt: Date.now()}, status: {neq: 3}}, include: ['jetty', 'mitra', 'referensiKontrak', 'si', 'transport', 'tujuanPltu', 'loading']}).subscribe(data => {
