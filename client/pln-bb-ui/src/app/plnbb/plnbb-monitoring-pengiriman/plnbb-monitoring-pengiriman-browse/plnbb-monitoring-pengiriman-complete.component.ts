...
@Component({
  selector: 'app-plnbb-monitoring-pengiriman-complete',
  template: `
            ...
            <ng-container *ngFor="let itemEntry of itemPltu.value">
              <tr>
                ...
                <td class="center aligned collapsing">
                  <a [routerLink]="['/plnbb', 'verifikasi-loading', itemEntry.loading.id, 'grant']">
...
export class PlnbbMonitoringPengirimanCompleteComponent implements OnInit {
  ...
  constructor(
    ...
    this.mitraApi.findOne({where: {userId: this.userApi.getCurrentId()}}).subscribe((mitra:Mitra) => {
      this.shippingApi.find({where: {mitraId: mitra.id, status: 3}, include: ['jetty', 'mitra', 'referensiKontrak', 'si', 'transport', 'tujuanPltu', 'loading', 'unloading']}).subscribe(data => {
