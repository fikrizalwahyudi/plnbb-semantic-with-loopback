...
@Component({
  selector: 'app-plnbb-monitoring-pengiriman-delayed',
  template: `
...
export class PlnbbMonitoringPengirimanDelayedComponent implements OnInit {
  ...
  constructor(
    ...
    this.mitraApi.findOne({where: {userId: this.userApi.getCurrentId()}}).subscribe((mitra:Mitra) => {
      this.shippingApi.find({where: {mitraId: mitra.id, laycanEndDate: {lte: Date.now()}, status: {neq: 3}}, include: ['jetty', 'mitra', 'referensiKontrak', 'si', 'transport', 'tujuanPltu', 'loading', 'unloading']}).subscribe(data => {
